...
function initAudioEffects() {
  ...
  const buttons = document.querySelectorAll('button, .cyber-button, a');
  ...
  // Removed icon-triggered audio bar

  // Removed icon hover listeners for audio bar
  

  document.body.appendChild(hoverSound);
  // Ensure audio is triggered on user gesture
  document.addEventListener('click', () => {
...
function initDarkModeToggle() {
  ...
  console.log(darkModePanel);

  const darkModeToggle = document.createElement('button');
  darkModeToggle.innerHTML = '<i class="fas fa-moon"></i> <span class="theme-label">Change Theme</span>';
  ...
  darkModeToggle.addEventListener('click', () => {
    ...
    if (document.body.classList.contains('light-mode')) {
      darkModeToggle.innerHTML = '<i class="fas fa-sun"></i> <span class="theme-label">Light Mode</span>';
    } else {
      darkModeToggle.innerHTML = '<i class="fas fa-moon"></i> <span class="theme-label">Dark Mode</span>';
    }
...
function initProjectFilters() {
  ...
  const categories = ['web', 'app', 'other'];
  
  projects.forEach(project => {
    // Assign random category for demo purposes
    const randomCategory = categories[Math.floor(Math.random() * categories.length)];
    project.setAttribute('data-category', randomCategory);
  });
...
function initMusicWidget() {
  ...
  container.className = 'music-widget';
  ...
  // Unlock autoplay on interaction
  document.addEventListener('click', () => {
    audio.play().catch(() => {});
  }, { once: true });
...
function initProjectFilters() {
  ...
  // Removed random category assignment — use manual data-category in HTML
...
const webhookUrl = 'YOUR_DISCORD_WEBHOOK_URL';  // Replace this with your actual webhook URL
...
function toggleTheme() {
  ...
  const next = current === "light" ? "dark" : "dark"; // optionally change to 'light' if needed
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedColor = localStorage.getItem("themeColor") || "#00ffea";

  // Apply saved theme and color
  document.documentElement.setAttribute("data-theme", savedTheme);
  updateColor(savedColor);

  // Connect toggle and color picker
  ...
  if (themeColor) {
    ...
    themeColor.addEventListener("input", (e) => updateColor(e.target.value));
  }
});
